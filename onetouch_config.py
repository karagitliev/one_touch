# General OneTouch application data
APPID = 6609898197243081281733444125044765054179316360618564706359682755
KIN = 2945322142
# General constants
TIMEOUT = 30
AUTH_TIMEOUT = 900

TOKEN = 78935936003423892015399874824155

# Registered client user info
USERNAME = 'a_t_xyTgiDz90Z'
PASSWORD = '8UP5slwW1E'

# Database files
USERS = 'database/users.txt'
USERS_DATA = 'database/users_data.txt'
USERS_PINS = 'database/users_pins.txt'
USERS_PAYMENTS = 'database/users_payments.txt'

# Logs
GLOBAL_LOG = 'logs/global.log'
ERR_LOG = 'logs/err.log'
AUTH_LOG = 'logs/auth.log'

# Authorisation
AUTH_START = 'https://demo.epay.bg/xdev/mobile/api/start?'
AUTH_VERIFY = 'https://demo.epay.bg/xdev/api/api/code/get?'
AUTH_GET_TOKEN = 'https://demo.epay.bg/xdev/api/api/token/get?'
# User info urls
USR_INF_GEN = 'https://demo.epay.bg/xdev/api/user/info?'
USR_INF_PINS = 'https://demo.epay.bg/xdev/api/user/info/pins?'
USR_INF_BALANCE = 'https://demo.epay.bg/xdev/api/user/info/pins/balance?'
# Payment
PAY_GET_ID = 'https://demo.epay.bg/xdev/api/payment/init?'
PAY_CHECK_INP = 'https://demo.epay.bg/xdev/api/payment/check?'
PAY_MONEY_SEND = 'https://demo.epay.bg/xdev/api/payment/send/user?'
PAY_CHECK_STATUS = 'https://demo.epay.bg/xdev/api/payment/send/status?'
